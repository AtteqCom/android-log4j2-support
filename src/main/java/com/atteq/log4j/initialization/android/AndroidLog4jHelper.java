...
public class AndroidLog4jHelper {
    ...
    public static void initialise(Context context, int configResource) {
        ...
        injectPlugins(
                "com.nasquan.opalsense.app.utility",
                new Class<?>[]{
                        AndroidLookup.class,
                        LogcatAppender.class
                }
        );
        LogManager.setFactory(new Log4jContextFactory(new AndroidContextSelector()));
    }

    static void injectPlugins(String packageName, Class<?>[] classes) {
        ...
        try {
            ...
            try {
                ...
                ConcurrentMap<String, Map<String, List<PluginType<?>>>> map = (ConcurrentMap<String, Map<String, List<PluginType<?>>>>) f
                        .get(reg);
    ...
    private static Map<String, List<PluginType<?>>> loadFromClasses(Class<?>[] classes) {
        ...
        for (final Class<?> clazz : classes) {
            ...
            final String mainElementName = plugin.elementType().equals(Plugin.EMPTY) ? plugin.name()
                    : plugin.elementType();
            ...
            @SuppressWarnings({"unchecked", "rawtypes"}) final PluginType<?> mainType = new PluginType(mainEntry, clazz, mainElementName);
            list.add(mainType);
            ...
            if (pluginAliases != null) {
                for (final String alias : pluginAliases.value()) {
                    ...
                    final String aliasElementName = plugin.elementType().equals(Plugin.EMPTY) ? alias.trim()
                            : plugin.elementType();
                    ...
                    @SuppressWarnings({"unchecked", "rawtypes"}) final PluginType<?> aliasType = new PluginType(aliasEntry, clazz, aliasElementName);
                    list.add(aliasType);
                }
            }
        }
